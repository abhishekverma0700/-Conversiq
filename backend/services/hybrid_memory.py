...
class HybridMemory:
    ...
    def build_chain(self, system_prompt: str, memory_context: Dict[str, object]):
        # Escape curly braces in all dynamic content to prevent ChatPromptTemplate errors
        safe_system_prompt = system_prompt.replace("{", "{{").replace("}", "}}")
        safe_summary = (memory_context.get("conversation_summary", "") or "").replace("{", "{{").replace("}", "}}")
        safe_entities = (memory_context.get("relevant_entities", "") or "").replace("{", "{{").replace("}", "}}")
        safe_recent = (memory_context.get("recent_messages_text", "") or "").replace("{", "{{").replace("}", "}}")
 
        formatted_system = HYBRID_SYSTEM_PROMPT_TEMPLATE.format(
            system_prompt=safe_system_prompt,
            conversation_summary=safe_summary,
            relevant_entities=safe_entities,
